...
%% Controller Options
kp  = diag([30,30,80,20,20,20]); % Proportional controller
...
legend('Generated trajectory','Simulated trajectory');
%% cable tension
figure(6)
plot(times,tau_c(:,:))
title('Cable tension \tau_c w.r.t. times');
grid on ; xlabel('time (s)') ; ylabel('Force (N)') ;
legend('Cable 1', 'Cable 2', 'Cable 3', 'Cable 4','Cable 5', 'Cable 6', 'Cable 7', 'Cable 8')

figure(7)
plot(times,tau_ff(:,:))
title('Cable tension \tau_{ff} w.r.t. times');
grid on ; xlabel('time (s)') ; ylabel('Force (N)') ;
legend('Cable 1', 'Cable 2', 'Cable 3', 'Cable 4','Cable 5', 'Cable 6', 'Cable 7', 'Cable 8')
...
function dx = odefun(t,x,data)
    ...
    We = data.We;
    Wg = data.Wg;  
    Wg = [0,0,-6 * 9.81,0,0,0]'; % test the robust of the system
